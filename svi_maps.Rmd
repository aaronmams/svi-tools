...
This Vignette assumes that you have already used the functions from [svi_censustract_functions.R]() to create the Census Tract level data frame with the 16 social vulnerability indicators.
...
The first step here is to set up the social vulnerability indicator data.  As explained in [this Vignette]() I don't really want to wait 30 minutes for the data to compile from API pings so I'm just going to load a version of the data that I stored:
